...
namespace VMware.Binding.WsTrust
{
    ...
    public class WsTrustClientMessageInspector : IClientMessageInspector
    {
        ...
        public object BeforeSendRequest(ref Message request, IClientChannel channel)
        {
            ...
            if (File.Exists("Vim25Service.dll"))
                productVersion = FileVersionInfo.GetVersionInfo("Vim25Service.dll").ProductVersion;
            else
                productVersion = FileVersionInfo.GetVersionInfo("STSService.dll").ProductVersion;

            string userAgent = string.Format(".Net Management SDK {0} " +
                               "C#/.NET Framework {1} ({2}; {3})",
                               productVersion, Environment.Version.ToString(),
                               Environment.OSVersion,
                               Environment.GetEnvironmentVariable("PROCESSOR_ARCHITECTURE"));
            HttpRequestMessageProperty realProp = new HttpRequestMessageProperty();
            realProp.Headers["User-Agent"] = userAgent;
