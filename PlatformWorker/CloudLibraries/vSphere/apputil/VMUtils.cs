...
namespace PlatformWorker
{
    ...
    {
        private PlatformWorker cb;
        private SvcConnection _connection;
        public VMUtils(PlatformWorker argCB)
        {
            cb = argCB;
        }
        ...
        public async Task<VirtualMachineConfigSpec> createVmConfigSpecAsync(String vmName,
                                                        String datastoreName,
                                                        int diskSizeMB,
                                                        ManagedObjectReference computeResMor,
                                                        ManagedObjectReference hostMor)
        {
            ConfigTarget configTarget = await getConfigTargetForHostAsync(computeResMor, hostMor);
            VirtualDevice[] defaultDevices = await getDefaultDevicesAsync(computeResMor, hostMor);
    ...
    */
        public async Task<ConfigTarget> getConfigTargetForHostAsync(ManagedObjectReference computeResMor, ManagedObjectReference hostMor)
        {
            ManagedObjectReference envBrowseMor = await cb.getServiceUtil().GetMoRefPropAsync(computeResMor, "environmentBrowser");
            ConfigTarget configTarget = await cb.getConnection().Service.QueryConfigTargetAsync(envBrowseMor, hostMor);
         ...
         * @throws Exception
         */
        public async Task<VirtualDevice[]> getDefaultDevicesAsync(ManagedObjectReference computeResMor,
                                                 ManagedObjectReference hostMor)
        {
            ManagedObjectReference envBrowseMor = await cb.getServiceUtil().GetMoRefPropAsync(computeResMor, "environmentBrowser");
            VirtualMachineConfigOption cfgOpt = await cb.getConnection().Service.QueryConfigOptionAsync(envBrowseMor, null, hostMor);
     ...
     * @throws Exception
     */
        public async Task<ManagedObjectReference> getHostFolderAsync(ManagedObjectReference dcmor)
        {
            ManagedObjectReference hfmor = await cb.getServiceUtil().GetMoRefPropAsync(dcmor, "hostFolder");
            return hfmor;
        }
        /**
       ...
       * @throws Exception
       */
        public async Task<ManagedObjectReference> getHostAsync(ManagedObjectReference hostFolderMor, String hostname)
        {
            ManagedObjectReference hostmor = null;
            if (hostname != null)
            {
                hostmor = await cb.getServiceUtil().GetDecendentMoRefAsync(hostFolderMor, "HostSystem", hostname);
            }
            else
            {
                hostmor = await cb.getServiceUtil().GetFirstDecendentMoRefAsync(hostFolderMor, "HostSystem");
            }
            return hostmor;
        }
