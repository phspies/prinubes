...
namespace PlatformWorker
{
    ...
    {
        private PlatformWorker _ci;
        private SvcConnection _connection;
        public ServiceUtil()
        {
        }
        public void Init(PlatformWorker ci)
        {
            _ci = ci;
            _connection = ci.getConnection();
        }
        public void ClientConnectCredentials()
        {
            try
            {
                ...
                if (url != null && username != null && password != null)
                {
                    ...
                    _connection.ConnectAsync(url, username, password);
                }
                else
                {
        ...
        public void ClientConnect(Cookie cookie)
        {
            try
            {
                ...
                if (url != null)
                {
                    _connection.ConnectAsync(url, cookie);
                }
                else
                {
      ...
      "TaskHistoryCollector"
        ...
        /// </returns>
        public async Task<ManagedObjectReference> GetDecendentMoRefAsync(ManagedObjectReference root, string type, string name)
        {
            ...
            ObjectContent[] ocary = await GetContentsRecursivelyAsync(null, root, typeinfo, true);
        ...
        public async Task<ManagedObjectReference> GetFirstDecendentMoRefAsync(ManagedObjectReference root, string type
        )
        {
            ArrayList morlist = await GetDecendentMoRefsAsync(root, type);
        ...
        public async Task<ArrayList> GetDecendentMoRefsAsync(ManagedObjectReference root, string type)
        {
            string[][] typeinfo =
        ...
        public async Task<ArrayList> GetDecendentMoRefsAsync(ManagedObjectReference root, String type, String[][] filter)
        {
            String[][] typeinfo = new String[][] { new String[] { type, "name" }, };
            ObjectContent[] ocary = await GetContentsRecursivelyAsync(null, root, typeinfo, true);
            ArrayList refs = new ArrayList();
            ...
            if (filter != null)
            {
                ArrayList filtermors = await filterMORAsync(refs, filter);
                return filtermors;
            }
            else
            {
        ...
        private async Task<ArrayList> filterMORAsync(ArrayList mors, String[][] filter)
        {
            ...
            for (int i = 0; i < mors.Count; i++)
            {
                ...
                for (int k = 0; k < filter.Length; k++)
                {
                    ...
                    String value = await getPropAsync(((ManagedObjectReference)mors[i]), prop);
                    if (reqVal == null)
                    {
                        continue;
        ...
        public async Task<string> getPropAsync(ManagedObjectReference obj, String prop)
        {
            String propVal = null;
            try
            {
                propVal = (String)(await GetDynamicPropertyAsync(obj, prop));
            }
            catch (Exception) { }
            return propVal;
        }
        ...
        public async Task<ObjectContent[]> GetAllContainerContentsAsync()
        {
            ObjectContent[] ocary = await GetContentsRecursivelyAsync(null, true);
        ...
        /// <returns>retrieved object contents</returns>
        public async Task<ObjectContent[]> GetContentsRecursivelyAsync(
        ...
        )
        {
            ...
            ObjectContent[] ocary = await GetContentsRecursivelyAsync(null, root, typeinfo, recurse);
        ...
        )
        {
            ...
            ObjectContent[] retoc = await retrievePropertiesExAsync(usecoll, new PropertyFilterSpec[] { spec });
        ...
        public async Task<ManagedObjectReference> GetMoRefPropAsync(ManagedObjectReference objMor, string propName)
        {
            if (objMor == null)
            {
            ...
            ObjectContent[] objcontent = await GetObjectPropertiesAsync(null, objMor, new string[] { propName });
        ...
        /// <returns>retrieved object contents</returns>
        public async Task<ObjectContent[]> GetObjectPropertiesAsync(
        ...
        )
        {
            ...
            return await retrievePropertiesExAsync(usecoll, new PropertyFilterSpec[] { spec });
        }

        public async Task<object> GetDynamicPropertyAsync(ManagedObjectReference mor, String propertyName)
        {
            ObjectContent[] objContent = await GetObjectPropertiesAsync(null, mor, new String[] { propertyName });
        ...
        public async Task<string> WaitForTaskAsync(ManagedObjectReference taskmor)
        {
            object[] result = await WaitForValuesAsync(
            ...
            {
                TaskInfo tinfo = (TaskInfo)(await GetDynamicPropertyAsync(taskmor, "info"));
        ...
        public async Task<object[]> WaitForValuesAsync(
        ...
        )
        {
            ...
            ManagedObjectReference filterSpecRef = await _connection.Service.CreateFilterAsync(_connection.PropCol, spec, true);
            ...
            while (!reached)
            {
                updateset = await _connection.Service.WaitForUpdatesAsync(_connection.PropCol, version);
            ...
            await _connection.Service.DestroyPropertyFilterAsync(filterSpecRef);
            return filterVals;
        }
        ...
        public async Task<ObjectContent[]> retrievePropertiesExAsync(ManagedObjectReference propertyCollector, PropertyFilterSpec[] specs)
        {
            return await retrievePropertiesExAsync(propertyCollector, specs, null);
        }

        public async Task<ObjectContent[]> retrievePropertiesExAsync(ManagedObjectReference propertyCollector, PropertyFilterSpec[] specs, int? maxObjects)
        {
            List<ObjectContent> objectList = new List<ObjectContent>();
            RetrieveResult result = (await _connection.Service.RetrievePropertiesExAsync(
                new RetrievePropertiesExRequest(
                    propertyCollector, specs,
                    new RetrieveOptions()
                    {
            ...
            if (result != null)
            {
                ...
                {
                    result = await _connection.Service.ContinueRetrievePropertiesExAsync(propertyCollector, token);
                    if (result != null)
                    {
                        token = result.token;
                        objectList.AddRange(result.objects.AsEnumerable());
        ...
        private async Task<ObjectContent[]> queryContainerViewAsync(ManagedObjectReference containerRoot, string morefType, string[] morefProperies)
        {
            List<ObjectContent> objectList = new List<ObjectContent>();
            var containerView = (await _connection.Service.CreateContainerViewAsync(
                new CreateContainerViewRequest(
                    _connection.ServiceContent.viewManager,
                    containerRoot,
            ...
            return await retrievePropertiesExAsync(_connection.PropCol, new PropertyFilterSpec[] { spec }, null);
        }
        ...
        /// <returns></returns>
        public async Task<Dictionary<ManagedObjectReference, Dictionary<string, object>>> getEntitiesInContainerByTypeAsync(ManagedObjectReference container, string morefType, string[] morefProperies)
        {
            ...
            ObjectContent[] contents = await queryContainerViewAsync(container, morefType, morefProperies);
        ...
        /// <param name="morefProperies"></param>
        /// <returns></returns>
        public async Task<Dictionary<ManagedObjectReference, Dictionary<string, object>>> getEntitiesByTypeAsync(string morefType, string[] morefProperies)
        {
            return await getEntitiesInContainerByTypeAsync(_connection.Root, morefType, morefProperies);
        }
        ...
        /// <param name="morefType"></param>
        /// <returns></returns>
        public async Task<Dictionary<string, ManagedObjectReference>> getNamedEntitiesByTypeAsync(string morefType)
        {
            return await getNamedEntitiesInContainerByTypeAsync(_connection.Root, morefType);
        }
        ...
        /// <param name="morefType"></param>
        /// <returns></returns>
        public async Task<Dictionary<string, ManagedObjectReference>> getNamedEntitiesInContainerByTypeAsync(ManagedObjectReference container, string morefType)
        {
            Dictionary<string, ManagedObjectReference> returnVal = new Dictionary<string, ManagedObjectReference>();
            ObjectContent[] contents = await queryContainerViewAsync(container, morefType, new string[] { "name" });
            foreach (ObjectContent content in contents)
            {
        ...
        /// <returns></returns>
        public async Task<ManagedObjectReference> getEntityByNameAsync(string morefType, string name)
        {
            return await getEntityInContainerByNameAsync(_connection.Root, morefType, name);
        }
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public async Task<ManagedObjectReference> getEntityInContainerByNameAsync(ManagedObjectReference container, string morefType, string name)
        {
            ObjectContent[] contents = await queryContainerViewAsync(container, morefType, new string[] { "name" });
            foreach (ObjectContent content in contents)
            {
