...
ServiceSettings serviceSettings = new ServiceSettings(_MYSQL_DATABASE: "prinubes_identity");
...
if (!args.Any(x => x.ToLower().Contains("testing")))
{
    ...
    builder.Services.AddDbContextPool<PrinubesIdentityDBContext>((serviceProvider, optionsBuilder) =>
    {
    ...
    //perform migrations
    builder.Services?.BuildServiceProvider().GetRequiredService<PrinubesIdentityDBContext>().MigrateIfRequired();
}
...
var kafkaProducerConfig = new ProducerConfig()
{
    ...
    EnableIdempotence = serviceSettings.KAFKA_IDEMPOTENCE,
    MessageSendMaxRetries = serviceSettings.KAFKA_RETRIES,
    BatchNumMessages = 1
};
...
builder.Services.AddSingleton<JwtBearer<PrinubesIdentityDBContext>>();
...
{
    x.EventsType = typeof(JwtBearer<PrinubesIdentityDBContext>);
